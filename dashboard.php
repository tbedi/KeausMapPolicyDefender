...
and crawl_results.crawl_id = (select max(id) from crawl) left join ( select
...
inner join crawl on crawl.id = crawl_results.crawl_id and crawl_results.crawl_id = (select max(id) 
from crawl where id != (select max(id) from crawl))
